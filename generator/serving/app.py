...
class Document(BaseModel):
    ...
    def validate_text(cls, value):
        is_ok, reason = _get_input_guardrail().validate(value)
        if not is_ok:
...
def ingest_mlflow():
    ...
    for doc in docs:
        if _get_input_guardrail().validate(doc.get("text", ""))[0] is False:
            continue
...
def agent_rag(payload: AgentRequest):
    ...
    result = None
    last_exc = None
    retryable_names = {"HTTPError", "RequestException"}
    ...
    for attempt in range(2):
        try:
            ...
            last_exc = exc
            is_retryable = isinstance(exc, (HTTPError, RequestException)) or type(exc).__name__ in retryable_names
            if not is_retryable or attempt == 1:
                break

    if result is None:
        fast = _query_with_rag(payload.query, payload.top_k)
        err_name = type(last_exc).__name__ if last_exc else "UnknownError"
        err_msg = str(last_exc)[:220] if last_exc else "sem detalhe"
        return {
            "query": payload.query,
            "answer": fast["answer"],
            "trace": [
                {
                    "step": 1,
                    "thought": "Execução ReAct falhou após retry; aplicado fallback resiliente via RAG direto.",
                    "action": "query_rag_exception_fallback",
                    "action_input": {"top_k": payload.top_k, "retry_attempts": 2},
                    "observation": f"Erro original do agente: {err_name} | detalhe: {err_msg}",
                }
            ],
        }
    ...
    answer_sanitized = _get_output_guardrail().sanitize(answer)
    _bad = {"reposta objetiva:", "resposta objetiva:", "reposta objetiva", "resposta objetiva", ""}
    if answer_sanitized.lower().rstrip(":").strip() in _bad or answer_sanitized.lower().startswith("reposta objetiv"):
        fast = _query_with_rag(payload.query, payload.top_k)
