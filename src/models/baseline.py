...
def main(args):
    ...
    tabela_metricas = pd.DataFrame(
        [
            {"Modelo": r["Modelo"], "MAE": r["MAE"], "RMSE": r["RMSE"], "MAPE (%)": r["MAPE (%)"]}
            for r in resultados
        ]
    )
    ...
    ax.axis("off")
    ax.table(cellText=tabela_metricas.values, colLabels=tabela_metricas.columns, loc="center")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--modelo", type=str, required=True, help="Caminho do modelo salvo (.keras)"
    )
    parser.add_argument(
        "--ticker", type=str, default="ITUB4.SA", help="Ticker para coletar dados atuais"
    )
