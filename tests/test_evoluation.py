...
class TestGoldenSet:
    ...
    def test_minimum_20_pairs(self, golden_set):
        ...
        assert len(golden_set) >= 20, f"Golden set tem apenas {len(golden_set)} pares. Mínimo: 20"
    ...
    def test_no_empty_expected_answers(self, golden_set):
        ...
        for i, item in enumerate(golden_set):
            assert item["expected_answer"].strip(), f"expected_answer vazio no item {i}"
    ...
    def test_minimum_one_context_per_item(self, golden_set):
        ...
        for i, item in enumerate(golden_set):
            assert len(item["contexts"]) >= 1, f"Item {i} sem contextos: '{item['query'][:50]}'"
    ...
    def test_tickers_referenced(self, golden_set):
        ...
        unique_tickers = set(
            t for t in tickers if t and t not in ("MULTI", "GENERIC", "SECTOR", "MACRO", "CDI")
        )
...
class TestRAGASEvaluation:
    ...
    def test_metrics_in_valid_range(self, tmp_path):
        ...
        with (
            patch("evaluation.ragas_eval.load_golden_set") as mock_load,
        ...
        for metric, value in metrics.items():
            if metric != "ragas_score_mean":
                assert 0.0 <= value <= 1.0, f"Métrica {metric}={value} fora do intervalo [0, 1]"
...
class TestLLMJudge:
    ...
    def test_business_criterion_present(self):
        ...
        assert (
            "decisão de investimento" in JUDGE_SYSTEM_PROMPT.lower()
            or "investimento" in JUDGE_SYSTEM_PROMPT.lower()
...
class TestDriftDetection:
    ...
    def test_run_drift_detection_returns_result(
        self, mock_report_cls, sample_financial_df, tmp_path
    ):
...
class TestPrometheusMetrics:
    ...
    def test_metrics_are_importable(self):
        ...
        assert LLM_JUDGE_SCORE is not None
    ...
    def test_track_request_increments_counter(self):
        ...
        before = REQUEST_COUNT.labels(
            ticker="VALE3", query_category="test", status="success"
        )._value.get()
    ...
    def test_drift_status_map_coverage(self):
        ...
        assert (
            DRIFT_STATUS_MAP["stable"] < DRIFT_STATUS_MAP["warning"] < DRIFT_STATUS_MAP["critical"]
        )
...
class TestABTest:
    ...
    def test_prompt_variants_exist(self):
        ...
        assert len(VARIANTS) >= 2, f"Apenas {len(VARIANTS)} variante(s) definida(s). Mínimo: 2"
